          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {form.role === 'createur' && (
              <div className="bg-gray-50 p-6 rounded-xl space-y-4">
                ...
                <div> <p className="text-gray-700 font-medium">Domaines d’activité :</p> <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2"> {domaines.map((d) => ( <label key={d} className="flex items-center gap-2 text-gray-700"> <input type="checkbox" checked={form.domaine.includes(d)} onChange={() => handleDomaineChange(d)} /> {d} </label> ))} </div> </div>
